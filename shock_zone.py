...
def shock_zone(sim_tree, X, Y, Z, Den, Temp, Vx, Vy, Vz, point, delta, cond3, check_cond = '3'):
    ...
    div_v = calc_div(sim_tree, X, Y, Z, Vx, Vy, Vz, point, delta)
    ...
    if np.logical_and(div_v<0, np.logical_and(cond2 > 0, cond3 == True)):
        return div_v
    else:
        return False
...
if __name__ == '__main__':
    ...
    # Find shock zone looping on all the cells and checking the 3 conditions from Schaal. Store data.
    shock_dirx = []
    shock_diry = []
    shock_dirz = []
    X_shock = []
    ...
    for i in range(len(X)):
        ...
        if math.isnan(np.linalg.norm(ds)):
            print('ds')
            continue
        cond3 = condition3(sim_tree, Press, Temp, point, ds, mach_min, gamma, step)
        shock = shock_zone(sim_tree, X, Y, Z, Den, Temp, VX, VY, VZ, point, step, cond3)
        
        if shock != False: # shock return the divergence of velocity
            X_shock.append(X[i])
            Y_shock.append(Y[i])
            Z_shock.append(Z[i])
