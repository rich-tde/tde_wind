...
which_obs = 'left_right_in_out_z' # 'left_right_z', 'all' or 'in_out_z'
snap = 76
rmin, rmax, Nray = Rt, 100*Rt, 500
origin = 'Rp'
more_cuts = ''
cut_wind = 'bern' # 'bern or 'orb_en' to pick the wind cells
weight_by = 'mass_weight' # 'mass_weight' or 'vol_weight' to weight the profiles by mass or volume
...
X, Y, Z, Rsph, Vol, Den, Mass, VX, VY, VZ, T, Press, IE_den, Rad_den, diss_den = \
    make_slices([X, Y, Z, Rsph, Vol, Den, Mass, VX, VY, VZ, T, Press, IE_den, Rad_den, diss_den], cut)
dim_cell = Vol**(1/3) 
diss = diss_den * Vol
indices_all = np.arange(len(X))
#%%
print(f'cutting wind cells with {cut_wind} condition')
cut, _, V_r = orb.pick_wind(X, Y, Z, VX, VY, VZ, Den, Mass, Press, IE_den, Rad_den, params, cond = cut_wind)
...
X, Y, Z, Rsph, Vol, Den, Mass, V_r, V_phi, T, Press, IE_den, Rad_den, diss, dim_cell = \
    make_slices([X, Y, Z, Rsph, Vol, Den, Mass, V_r, V_phi, T, Press, IE_den, Rad_den, diss, dim_cell], cut)       
...
for j, cond in enumerate(cond_sec):
    ...
    for i, r in enumerate(ray_array): 
        ...
        ray_diss = diss[idx]
        if weight_by == 'mass_weight':
            print('weighting by mass')
            weight = ray_m
        elif weight_by == 'vol_weight':
            print('weighting by volume')
            weight = ray_vol
        ...
        img1 = ax1.scatter(X_toplot[np.abs(Z_toplot)<Rt]/Rt, Y_toplot[np.abs(Z_toplot)<Rt]/Rt, s = 2, c = ray_d[np.abs(Z_toplot)<Rt]*prel.den_converter, norm = colors.LogNorm(vmin = 1e-12, vmax = 1e-7), cmap = 'rainbow')
        img2 = ax2.scatter(X_toplot[np.abs(Y_toplot)<Rt]/Rt, Z_toplot[np.abs(Y_toplot)<Rt]/Rt, s = 2, c = ray_d[np.abs(Y_toplot)<Rt]*prel.den_converter, norm = colors.LogNorm(vmin = 1e-12, vmax = 1e-7), cmap = 'rainbow')
        img3 = ax3.scatter(X_toplot[np.abs(Z_toplot)<Rt]/Rt, Y_toplot[np.abs(Z_toplot)<Rt]/Rt, s = 2, c = np.abs(Vphi_toplot[np.abs(Z_toplot)<Rt])/ray_V_r[np.abs(Z_toplot)<Rt], norm = colors.LogNorm(vmin = 1e-1, vmax = 1e1), cmap = 'coolwarm')
        img4 = ax4.scatter(X_toplot[np.abs(Y_toplot)<Rt]/Rt, Z_toplot[np.abs(Y_toplot)<Rt]/Rt, s = 2, c = ray_diss[np.abs(Y_toplot)<Rt]*prel.en_converter/prel.tsol_cgs, norm = colors.LogNorm(vmin = 1e32, vmax = 1e37), cmap = 'viridis')

        ray_t = (ray_rad_den * prel.en_den_converter / prel.alpha_cgs)**(1/4) 
        L_adv =  ray_V_r * ray_rad_den
        t_prof[i] = np.sum(ray_t*weight) / np.sum(weight)
        v_rad_prof[i] = np.sum(ray_V_r*weight) / np.sum(weight)
        d_prof[i] = np.sum(ray_d*weight)/ np.sum(weight)
        ...
        diss_prof[i] = np.sum(ray_diss*weight)/ np.sum(weight)
...
cbar.set_label(r'Diss rate (erg/s)', fontsize = 20)
for ax in [ax1, ax2, ax3, ax4]:
    ax.set_xlim(-10, 10)
    ax.set_ylim(-10, 10)
    ax.set_xlabel(r'X/$r_{\rm t}$')
plt.suptitle(f'All wind points in slice ±Rt in snap {snap} with origin = {origin}', fontsize = 22)
plt.tight_layout()
plt.savefig(f"{abspath}/Figs/Test/wind/rprof_scatt{snap}_origin{origin}{more_cuts}.png", dpi = 300)
out_path = f"{abspath}/data/{folder}/wind/r_profile/{cut_wind}/r_prof{snap}_{which_obs}_origin{origin}{more_cuts}_{weight_by}.npy"
...
if plot:
    ...
    fig, ((axd, axMtot), (axV, axD), (axM, axN)) = plt.subplots(3, 2, figsize=(20, 20)) 

    all_axes = [axd, axV, axM, axN, axMtot, axD]
    ...
    profiles = np.load(f'{abspath}/data/{folder}/wind/r_profile/{cut_wind}/r_prof{snap}_{which_obs}_origin{origin}{more_cuts}_{weight_by}.npy', allow_pickle=True).item()
    for i, lab in enumerate(profiles.keys()):
        if label_obs[i] in ['South pole', 'North pole']:
            continue 
        lab_plot = lab
        ...
        diss_prof = profiles[lab]['diss_prof']
        ...
        r_plot, d, d_mean, v_rad, t, Mdot, ratio_un, ratio_Mass, Mdotmean, diss_prof = \
            make_slices([r_arr, d, d_mean, v_rad, t, Mdot, ratio_un, ratio_Mass, Mdotmean, diss_prof], not_zero)
        Nwind_cells, Ntot_cells, Mass_wind, Mass_tot = \
            make_slices([Nwind_cells, Ntot_cells, Mass_wind, Mass_tot], not_zero)
        Mdot_approx = np.diff(Mass_wind)/np.diff(r_plot) * v_rad[:-1]
        ...
        axM.plot(r_plot*norm, Mdot/Medd_sol, color = colors_sec, linewidth = 2, label = r'$\rho v_{\rm r}$' if i == 0 else None)
        # axM.plot(r_plot*norm, Mdotmean/Medd_sol, color = colors_sec, linewidth = 2, ls = '--', label = r'$\langle \rho v_{\rm r} \rangle$' if i == 0 else None)
        # axM.plot(r_plot[:-1]*norm, Mdot_approx/Medd_sol, color = colors_sec, linewidth = 2, ls = ':', label = r'dM/dr v$_{\rm r}$' if i == 0 else None)
        axMtot.plot(r_plot*norm, Mass_wind, color = colors_sec, linewidth = 2)
        axD.plot(r_plot*norm, diss_prof * prel.en_converter/prel.tsol_cgs, color = colors_sec, linewidth = 2)
        axN.plot(r_plot*norm, Nwind_cells, color = colors_sec, linewidth = 2)

    axV.axhline(v_esc_kms, c = 'k', ls = 'dotted')
    axd.set_ylabel(r'$\rho$ (g/cm$^3$)', fontsize = 35)
    axV.set_ylabel(r'v$_{\rm r}$ (km/s)', fontsize = 35)
    axM.set_ylabel(r'$\dot{M} (\dot{M}_{\rm Edd})$', fontsize = 35)
    axN.set_ylabel(r'N$_{\rm wind}$', fontsize = 35)
    axMtot.set_ylabel(r'M$_{\rm wind}$ (M$_{\rm \odot}$)', fontsize = 35)
    axD.set_ylabel(r'$\dot{E}_{\rm diss}$ (erg/s)', fontsize = 35)
    for ax in all_axes: 
        ...
        ax.grid()
    axM.set_xlabel(r'$r /r_{\rm t}$', fontsize = 35)     
    axN.set_xlabel(r'$r /r_{\rm t}$', fontsize = 35)  
    axd.legend(fontsize = 25)
    axM.legend(fontsize = 25)
    fig.tight_layout()
    # plt.suptitle(f'Origin = {origin}, {cut_wind}, t = {np.round(tfb,2)} ' + r'$t_{\rm fb}$', fontsize = 22)
    plt.savefig(f"{abspath}/Figs/Test/wind/rprof_{cut_wind}_{snap}_origin{origin}{more_cuts}_{weight_by}.png", dpi = 300)
