...
if alice:
    ...
    for idx_s, snap in enumerate(snaps):
        # Load the data
        photo = np.loadtxt(f'{pre_saving}/photo/{check}{extr}_photo{snap}.txt')
        x, y, z, vol, den, Temp = photo[0], photo[1], photo[2], photo[3], photo[4], photo[5]
        dim_cell = (vol)**(1/3)
        # Midplane
        mid = np.abs(z) < dim_cell
        x_mid, y_mid, z_mid, den_mid, Temp_mid = make_slices([x, y, z, den, Temp], mid)
        plt.figure(figsize = (10,5))
        plt.scatter(x_mid/apo, y_mid/apo, c = 'r', s = 20)
        plt.scatter(0,0,c= 'k', marker = 'x', s=80)
        plt.xlim(-4,0.5)
        plt.ylim(-2,2)
        plt.xlabel(r'X [$R_a$]', fontsize = 18)
        plt.ylabel(r'Y [$R_a$]', fontsize = 18)
