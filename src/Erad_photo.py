...
for idx_s, snap in enumerate(snaps):
    ...
    # Save the data ----------------------------------------------------------------    
    Rad_den_R_snap.append(Rad_den_tot)

eng.exit()

if save:
    np.save(f'{pre_saving}/{check}{extr}_Radden.npy', Rad_den_R_snap)
    np.save(f'{pre_saving}/{check}{extr}_R_Radden.npy', r_arr)
    np.save(f'{pre_saving}/{check}{extr}_time_Radden.npy', [snaps, tfb])
