""" If what = 'compare_res' is True, it plots the WH and #cells as function of the anomaly for different res at different times.
...
If what = 'onlysection' is True, it plots only the midplane with stream and width of the chosen res, with scatter plot for density/mass.
If what = 'section4' is True, it plots the midplane and transverse plane, mass distribution and T-density scatter.
If what = 'max_compr' is True, it plots the res compare_res of H_min and ratio between before and after the compression.
"""
...
if what == 'compare_res':
    ...
    for i, time in enumerate(wanted_time): 
        ...
        for i, check in enumerate(checks):
            ...
            data_width = np.load(f'{abspath}/data/{folder}/WH/{mass_perc}/wh_{mass_perc}{check}_{snap}.npz', allow_pickle=True)
...
if what in ['single_snap_behavior', 'compare_perc', 'section4', 'onlysection', 'ev_fixedTheta', 'follow_orbit']:
    check = 'LowResNewAMR'
    folder = f'R{Rstar}M{mstar}BH{Mbh}beta{beta}S60n{n}{compton}{check}'
    ...
    if what == 'ev_fixedTheta':
        theta_wanted = 0
        width_t = np.zeros(len(snaps))
        height_t = np.zeros(len(snaps))
        for i, snap in enumerate(snaps):
            data_width = np.load(f'{abspath}/data/{folder}/WH/{mass_perc}/wh_{mass_perc}{check}_{snap}.npz', allow_pickle=True)
            theta_wh = data_width['theta_wh']
            idx_theta_wanted = np.argmin(np.abs(theta_wh - theta_wanted))
            width_t[i] = data_width['width'][idx_theta_wanted]
            height_t[i] = data_width['height'][idx_theta_wanted]

        fig, ax = plt.subplots(1,1, figsize = (10,7))
        ax.plot(tfb, width_t, label = 'Width', c = 'dodgerblue')
        ax.plot(tfb, height_t, label = 'Height', c = 'orange')
        ax.set_xlabel(r'Time [t$_{\rm fb}$]')
        ax.set_ylabel(r'Stream size [$R_\odot$]')
        ax.set_xlim([0.08, np.max(tfb)])
        ax.set_ylim([0.1, 10])
        ax.set_yscale('log')
        ...
        ax.grid()
        plt.suptitle(r'Stream size at $\theta$ = ' + f'{np.round(theta_wanted, 2)} rad', fontsize = 20)
        plt.tight_layout()

    else:
        snap = 240
        time = tfb[np.argmin(np.abs(snaps-snap))]
        ...
        if what == 'compare_perc': 
            data_width09 = np.load(f'{abspath}/data/{folder}/WH/0.9/wh_0.9{check}_{snap}.npz', allow_pickle=True)
            theta_wh09 = data_width09['theta_wh']
            x_low_width09 = data_width09['X_low_w']
            x_up_width09 = data_width09['X_up_w']
            y_low_width09 = data_width09['Y_low_w']
            y_up_width09 = data_width09['Y_up_w']
            width09 = data_width09['width']
            N_width09 = data_width09['N_width']
            height09 = data_width09['height']
            N_height09 = data_width09['N_height']
        
        if what == 'single_snap_behavior' or what == 'compare_perc': 
            ...
            # Plot stream (zoom out and in on theorbital plane) 
                ...
                for ax in [ax1, ax2]: 
                    ...
                    # ax.plot(x_stream[-30:]/Rt, y_stream[-30:]/Rt, c = 'k')
                    ax.plot([x_stream[0]/Rt, x_stream[-1]/Rt] , [y_stream[0]/Rt, y_stream[-1]/Rt], c = 'k')
                    ax.plot(x_low_width/Rt, y_low_width/Rt, ls = '--', c = 'k')
                    ax.plot(x_up_width/Rt, y_up_width/Rt, ls = '--', c = 'k')
                ...
                ax2.set_xlim(-2, 2)
                ax2.set_ylim(-2, 2)
                plt.suptitle(r'Stream at t/t$_{\rm fb}$ = ' + str(np.round(time,2)) + f', check: {check}', fontsize = 16)

            # Plot width and height and number of cells
            fig, (ax1,ax2)= plt.subplots(2,1, figsize = (9,12))
            ax1.plot(theta_wh * radians, width, c = 'dodgerblue', label = f'mass enclosed {mass_perc}%' if what == 'compare_perc' else 'Width')
            img = ax2.plot(theta_wh * radians, N_width, c = 'dodgerblue')#, cmap = 'rainbow', norm = colors.LogNorm(vmin = 1, vmax = 15))
            ax1.set_ylabel(r'Stream size [$R_\odot$]')
            ax2.set_ylabel(r'N$_{\rm cells}$')
            ax1.plot(theta_wh * radians, height, c = 'orange' , label = f'' if what == 'compare_perc' else 'Height')
            img = ax2.plot(theta_wh * radians, N_height, c = 'orange')#, cmap = 'rainbow', norm = colors.LogNorm(vmin = 0.5, vmax = 10))
            if what == 'compare_perc':
                ax1.plot(theta_wh09 * radians, width09, c = 'blue', label = f'mass enclosed 0.9%')
                ax2.plot(theta_wh09 * radians, N_width09, c = 'blue')
                ax1.plot(theta_wh09 * radians, height09, c = 'red')
                ax2.plot(theta_wh09 * radians, N_height09, c = 'red')
            ...
            fig.suptitle(r't/t$_{fb}$ = ' + str(np.round(time,2)), fontsize = 20)
            plt.tight_layout()
        ...
        if what == 'section4' or what == 'onlysection':
            ...
            if what == 'onlysection':
                ...
                indeces_boundary = np.load(f'{abspath}/data/{folder}/WH/{mass_perc}/indeces_boundary_0.8{check}_{snap}.npy', allow_pickle=True)
                ...
                enclosed_cells = np.load(f'{abspath}/data/{folder}/WH/{mass_perc}/enclosed/indeces_enclosed_0.8{check}_{snap}.npy', allow_pickle=True)
                ...
                plt.suptitle(r'$\alpha$ = ' + f'{np.round(theta_wh[idx], 2)}, {check}, t =  {np.round(time,2)}' + r' $t_{\rm fb}$, number cells $\Delta$ = ' + f'{N_width[idx]}, H = {N_height[idx]}', fontsize = 25)
