...
def dRsi_dRp(Mbh, mstar, Rstar, beta):
    Rp = pericentre(Mbh, mstar, Rstar, beta)
    a = semimajor_axis(Mbh, mstar, Rstar)
    phi, _ = precession_analyt(Mbh, mstar, Rstar, beta)
    e = eccentricity(Rp,a)
...
plt.scatter(phi_oneBH[0], Rsi_oneBH[0]/Ra_oneBH[0], c='k', marker = 's')
plt.scatter(phi_oneBH[-1], Rsi_oneBH[-1]/Ra_oneBH[-1], c='k', marker = 'o')
plt.plot(phi_oneBH, Rsi_oneBH/Ra_oneBH, c='k')
plt.xlabel(r'$\phi$ [rad]', fontsize = 18)
plt.ylabel(r'$R_{SI}/R_a$', fontsize = 18)
plt.text(0.05, 0.4, 'square = starting point \ndot = ending point', fontsize = 15)
plt.title(f'$M_h = 10^{m[isel]} M_\odot, R_\star = {Rstar} R_\odot, M_\star = {mstar} M_\odot,  R_\star = {Rstar} R_\odot$, ' + r'$\beta\in$ [' + f'{np.round(beta_many[0], 1)}, {np.round(beta_many[-1])}]')
plt.grid()
