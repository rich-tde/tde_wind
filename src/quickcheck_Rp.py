...
if do:
    ...
    for i, snap in enumerate(snaps):
        ...
        if alice:
            pathsnap = f'/home/martirep/data_pi-rossiem/TDE_data/{folder}/snap_{snap}'
        else:
            pathsnap = f'{abspath}/TDE/{folder}/{snap}'
        data = make_tree(pathsnap, snap, energy = False)
        ...
        print(f'{Rp_min_size[i]},{Rp_avg_size[i], scale_height[i]}', flush=False)
        sys.stdout.flush()

    with open(f'{abspath}/data/{folder}/Rp_statistics{check}.txt', 'a') as file:
        # wrtie tfb
        file.write(f'# t/tfb \n' + ' '.join(map(str, tfb)) + '\n')
    ...
    checks = ['LowRes', '', 'HiRes']
    colors_check = ['C1', 'yellowgreen', 'darkviolet']
    labels = ['Low', 'Fid', 'High']
    fig, (ax1,ax2) = plt.subplots(2, 1, figsize=(7, 10))
    for i, check in enumerate(checks):
        folder = f'R{Rstar}M{mstar}BH{Mbh}beta{beta}S60n{n}{compton}{check}'
        tfb, Rp_min_size, Rp_avg_size, scale_height = \
            np.loadtxt(f'{abspath}/data/{folder}/Rp_statistics{check}.txt', skiprows = 1)
        num_cells = scale_height / Rp_avg_size
    
        # ax1.plot(tfb, Rp_min_size, color = colors_check[i], label = labels[i])
        ax1.plot(tfb, Rp_avg_size, color = colors_check[i], label = labels[i])
        ax1.set_yscale('log')
        ax1.set_ylabel(r'Size at $R_{\rm p} [R_\odot$]')
        ax2.plot(tfb, num_cells, color = colors_check[i], label = labels[i])
        ax2.set_xlabel(r't [t$_{\rm fb}$]')
        for ax in (ax1, ax2):
            ax.grid()
            # make longer the y ticks
            ax.tick_params('both', length=7, width=1, which='major')
            ax.tick_params('y', length=5, width=1, which='minor')      
        ax2.set_ylabel(r'Number of cells in H$_{\rm z} [R_{\rm p}]$')
        ax1.legend(fontsize = 14)
        plt.tight_layout()
        plt.savefig(f'{abspath}/Figs/talk/Rp_statistics.png', bbox_inches='tight')
