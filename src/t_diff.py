...
for i in range(len(observers_xyz)):
    ...
    ray_vz = VZ[idx]
    # plt.figure()
    # img = plt.scatter(xyz2[:,1], xyz2[:,2], c = xyz2[:,0], alpha = 0.2, cmap = 'jet', marker = 's', label = 'What we want', vmin = Rt-5, vmax = Rt+5)
    # img = plt.scatter(ray_y, ray_z, c = ray_x, cmap = 'jet', label = 'From simulation', vmin = Rt-5, vmax = Rt+5)
    ...
    ax1.plot(ray_z/apo, tdiff_cumulative, c = 'k')
    ax1.set_xlabel(r'$Z [R_{\rm a}]$')
    ax1.set_ylabel(r'$t_{\rm diff}/t_{\rm fb}$')
    ax1.set_yscale('log')
    ax1.set_xlim(0, 25/apo) # a bit further than the Rtrapp
    img = ax2.scatter(ray_z/apo, los, c = ctau/ray_vz, cmap = 'rainbow', vmin = 1e-5, vmax = 1)
    cbar = plt.colorbar(img)
    cbar.set_label(r'c$\tau^{-1}/V_z$')
    ax2.set_xlabel(r'$Z [R_{\rm a}]$')
    ax2.set_ylabel(r'$\tau$')
    ax2.set_xlim(0, 50/apo)
    ax2.set_ylim(0.1,1)
    plt.tight_layout()
