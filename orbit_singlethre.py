...
def find_transverse_com(x_data, y_data, z_data, dim_data, den_data, mass_data, theta_arr, Rt, Rp):
...
def find_single_boundaries(x_data, y_data, z_data, dim_data, den_data, mass_data, indeces_orbit, idx, params):
...
if __name__ == '__main__':
    ...
    if make_orbit:
        ...
        plt.plot(x_stream, y_stream, c = 'k', label = 'COM stream')
        plt.xlim(-300,20)
        plt.ylim(-60,60)
        plt.grid()
        if compare:
            img = plt.scatter(X_midplane, Y_midplane, c = Mass_midplane, s = 1, cmap = 'viridis', vmin = 1e-10, vmax = 4e-10, alpha = 0.2)
    ...
    if compare:
        ...
        indeces_stream2, indeces_boundary2, x_T_width2, w_params2, h_params2, theta_arr2  = follow_the_stream(data2.X, data2.Y, data2.Z, dim_cell1, data2.Den, data2.Mass, theta_arr, path = file2)

        #%%
        print(indeces_boundary)
