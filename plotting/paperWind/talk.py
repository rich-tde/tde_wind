...
for j in range(2):
    ...
    all_axes[3][j].set_ylim(5e-1, 2e3)
    for i in range(4):
        all_axes[i][j].grid()
...
all_axes[3][0].text(57, 9, r'$L \propto r^{-2/3}$', fontsize = 25, color = 'gray', rotation = -15)            
handles_color = []
labels_color = []
        ...
        for i, lab in enumerate(profiles.keys()):
            ...
            if isoent == 'isoent':
                ...
                if what_varies == 'r':
                    ...
                    L_adv =  (4 * np.pi * r_chosen_theta**2) / area * profiles[lab]['L_adv_prof'] 
            else:
            ...
            if what_varies == 'r':
                r_plot, d, v_rad,  Mdot, L_kin, L_adv = \
            ...
            if np.logical_and(np.logical_and(what_varies =='r' , which_part == 'wind'), rph_nonzero_medians[i] != 0):
                ...
                all_axes[3][w].scatter(r_plot[idx_rph]*norm, L_adv[idx_rph]/Ledd_sol, marker = 'o', s = 100, color = colors_sec)
