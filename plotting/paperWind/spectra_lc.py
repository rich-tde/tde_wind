...
def lc_from_fit(folder, check, choice, group):
    ...
    for s, snap in enumerate(snaps_fld):
        ...
        for i_idx, idx in enumerate(indices_sorted):
            ...
            fit = pmodel.fit(Lum[idx_opt], n = freqs[idx_opt], R=1e13, T=1e4)
            ...
            fit = pmodel.fit(Lum[idx_UV], n = freqs[idx_UV], R=1e13, T=1e4)
            Rfit = fit.params['R'].value
            Tfit = fit.params['T'].value
            BBfit = [lumfit(freq, Rfit, Tfit) for freq in freqs]
            Lum_UV[i_idx] = sci.trapezoid(BBfit, freqs) #, initial=0)

            fit = pmodel.fit(Lum[idx_Xray], n = freqs[idx_Xray], R=1e13, T=1e4)
            Rfit = fit.params['R'].value
            Tfit = fit.params['T'].value
            BBfit = [lumfit(freq, Rfit, Tfit) for freq in freqs]
    ...
    if group == 'sections': # each panel show a spherical sector
        ...
        for k, obs in enumerate(np.arange(len_plot)):
            ax_L[k].plot(tfb, Lum_op_mean[k], label = 'Optical', c = colors_obs[k], linewidth = 3)
            ax_L[k].plot(tfb, Lum_UV_mean[k], label = f'UV', c = colors_obs[k], linewidth = 3, ls = '--')
            ax_L[k].plot(tfb, Lum_Xray_mean[k], label = f'Xray', c = colors_obs[k], linewidth = 3, ls = ':')
    ...
    for s, snap in enumerate(snaps):
        ...
        for i_idx, idx in enumerate(indices_sorted):
            ...
            if len(idx) == 1:
                ...
                Lum = np.mean(L_col[idx], axis = 0)
            # fit = pmodel.fit(Lum[idx_Xray], n = freqs[idx_Xray], R=1e13, T=1e4)
            fit = pmodel.fit(Lum, n = freqs, R=1e13, T=1e4)
            Rfit = fit.params['R'].value
            Tfit = fit.params['T'].value
            BBfit = np.array([lumfit(freq, Rfit, Tfit) for freq in freqs])
            # print(fit.fit_report())
            print(f'Observer {label_obs[i_idx]}: Tfit = {Tfit:.2e} K, Rfit = {Rfit/prel.Rsol_cgs:.2e} rsol')
            ax[s].plot(x_value, freqs * Lum, label = f'{label_obs[i_idx]}', c = colors_obs[i_idx], linewidth = 3) #ls = lines_obs[i_idx]
            ax[s].plot(x_value, freqs * BBfit, c = colors_obs[i_idx], ls = '--', linewidth = 2, label = f'BB fit' if i_idx == 0 else None)
