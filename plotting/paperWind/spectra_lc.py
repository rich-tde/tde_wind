...
# def lc_from_fit(folder, check, choice, group):
#     # Load
#     pre_saving = f'{abspath}/data/{folder}'
#     freqs = np.loadtxt(f'{pre_saving}/spectra/freqs.txt')
#     lambda_val = prel.c_cgs / freqs
...
#         Lum_op, Lum_UV, Lum_Xray = np.zeros(len(indices_sorted)), np.zeros(len(indices_sorted)), np.zeros(len(indices_sorted))
...
#             fit = pmodel.fit(Lum[idx_opt], n = freqs[idx_opt],  params=params)
#             Rfit = fit.params['R'].value
#             Tfit = fit.params['T'].value
#             BBfit = [lumfit(freq, Rfit, Tfit) for freq in freqs]
#             Lum_op[i_idx] = sci.trapezoid(BBfit, freqs) #, initial=0)

#             fit = pmodel.fit(Lum[idx_UV], n = freqs[idx_UV],  params=params)
#             Rfit = fit.params['R'].value
#             Tfit = fit.params['T'].value
#             BBfit = [lumfit(freq, Rfit, Tfit) for freq in freqs]
#             Lum_UV[i_idx] = sci.trapezoid(BBfit, freqs) #, initial=0)

#             fit = pmodel.fit(Lum[idx_Xray], n = freqs[idx_Xray],  params=params)
#             Rfit = fit.params['R'].value
#             Tfit = fit.params['T'].value
#             BBfit = [lumfit(freq, Rfit, Tfit) for freq in freqs]
#             Lum_Xray[i_idx] = sci.trapezoid(BBfit, freqs) #, initial=0)

#         Lum_op_mean.append(Lum_op)
#         Lum_UV_mean.append(Lum_UV)
#         Lum_Xray_mean.append(Lum_Xray)
...
#         Lum_op_MG_mean = np.mean(L_colMG[:, 1:3], axis = 1)
#         Lum_UV_MG_mean = np.mean(L_colMG[:, 3:5], axis = 1)
#         Lum_Xray_MG_mean = np.mean(L_colMG[:, 8:], axis = 1)
#         # Lum_op_MG_mean = L_colMG[:, 1]
#         # Lum_UV_MG_mean = L_colMG[:, 3]
#         # Lum_Xray_MG_mean = L_colMG[:, 8]

#         Lum_op_MG.append([np.mean(Lum_op_MG_mean[idx]) for idx in indices_sorted_MG])
#         Lum_UV_MG.append([np.mean(Lum_UV_MG_mean[idx]) for idx in indices_sorted_MG])
#         Lum_Xray_MG.append([np.mean(Lum_Xray_MG_mean[idx]) for idx in indices_sorted_MG])
...
#             axes[k].plot(tfb, Lum_op_mean[k], label = 'Optical', c = colors_obs[k], linewidth = 3)
#             axes[k].plot(tfb, Lum_UV_mean[k], label = f'UV', c = colors_obs[k], linewidth = 3, ls = '--')
#             axes[k].plot(tfb, Lum_Xray_mean[k], label = f'Xray', c = colors_obs[k], linewidth = 3, ls = ':')
#             # axes[k].plot(time_MG, Lum_Xray_MG[k], label = f'Xray', c = colors_obs[k], linewidth = 3, ls = ':')
...
#             if k == 3:
#                 continue
#             ax_op.plot(tfb, Lum_op_mean[k], label = f'{obs}', c = colors_obs[k], linewidth = 3)
#             ax_UV.plot(tfb, Lum_UV_mean[k], label = f'This work' if k == 2 else None, c = colors_obs[k], linewidth = 3)
#             ax_Xray.plot(time_MG, Lum_Xray_MG[k], c = colors_obs[k], ls = '--' if group == 'bandsMG' else '-', linewidth = 3)
#             if group == 'bandsMG':
#                 ax_Xray.plot(tfb, Lum_Xray_mean[k], c = colors_obs[k], linewidth = 3)
...
#         ax.plot(tfb, Lum_fld, c = 'k', ls = '--', linewidth = 2, label = 'bolometric')
#         ax.set_yscale('log')
...
#     plt.tight_layout()
    # plt.savefig(f'{abspath}/Figs/2.paperWind/LCs_{choice}_fromFit.pdf', dpi=300)
...
def plot_spectra(folder, check, snaps, x_axis, choice, in_moll = False):
    ...
    for s, snap in enumerate(snaps):
        ...
        if in_moll:
            ...
            for i in range(len(L_col)):
                ...
                Lum_op[i] = np.mean(Lum_freq[idx_opt])
                # Lum_UV[i] = np.mean(Lum_freq[idx_UV])
                Lum_Xray[i] = np.mean(Lum_freq[idx_Xray])
        ...
        for i_idx, idx in enumerate(indices_sorted):
            ...
            ax[s].plot(x_value, freqs * Lum, label = f'{label_obs[i_idx]}'  if s == 0 else None, c = colors_obs[i_idx],linewidth = 3) #ls = lines_obs[i_idx]
            # if i_idx == len(indices_sorted)-2:
            #     ax[s].plot(x_value, freqs * BBfit, c = 'k', ls = '-.', linewidth = 2, label = f'BB fit') # just for the legend
        ...
        if len(indices_sorted) < 6:
            for i_idx, idx in enumerate(indices_sorted): 
                ...
                ax[s].plot(x_value, freqs * BBfit, c = colors_obs[i_idx], ls = '-.', label = f'T={Tfit[i_idx]*1e-4:.1f}' + r' $\times 10^4$ K' if s < 2 else f'T={Tfit[i_idx]*1e-3:.1f}' + r' $\times 10^3$ K', linewidth = 2)
    ...
    fig_sp.savefig(f'{abspath}/Figs/2.paperWind/spectra_{choice}.pdf', dpi=300)
...
def plot_light_curves(folder, check, choice, group = 'bands'):
    ...
    for s, snap in enumerate(snaps_fld):
        ...
        Lum_op_mean.append(np.mean(Lum_op[indices_sorted], axis = 1))
        Lum_UV_mean.append(np.mean(Lum_UV[indices_sorted], axis = 1))
        Lum_Xray_mean.append(np.mean(Lum_Xray[indices_sorted], axis = 1))
    ...
    if group == 'sections': # each panel show a spherical sector
        ...
        for k in np.arange(len(axes)):
            ...
            axes[k].plot(tfb, Lum_op, label = 'Optical', c = colors_obs[k], linewidth = 3)
            axes[k].scatter(tfb[np.argmax(Lum_op)], np.max(Lum_op), c = colors_obs[k], s = 200, marker = 'd')
            axes[k].plot(tfb, Lum_UV, label = f'UV', c = colors_obs[k], linewidth = 3, ls = '--')
            axes[k].scatter(tfb[np.argmax(Lum_UV)], np.max(Lum_UV), c = colors_obs[k], s = 200, marker = 'd')
            axes[k].plot(time_MG, Lum_XrayMG, label = f'Xray', c = colors_obs[k], linewidth = 3, ls = ':')
            axes[k].scatter(time_MG[np.argmax(Lum_XrayMG)], np.max(Lum_XrayMG), c = colors_obs[k], s = 200, marker = 'd')
            axes[k].text(0.1, L_max/5, f'{label_obs[k]}', fontsize = 26)
        original_ticks = axes[0].get_xticks()
        axes[0].legend(fontsize = 25)
        axes[0].set_ylabel(r'$\nu L_{\nu}$ (erg/s)', fontsize = 30)
    ...
    if group == 'bands' or group == 'bandsMG': # each panel show a band
        ...
        for k, obs in enumerate(label_obs):
            ...
            Lum_UV = Lum_UV_mean[k] 
            Lum_XrayMG = Lum_Xray_MG[k]
            ax_op.plot(tfb, Lum_op, label = f'{obs}', c = colors_obs[k], linewidth = 3)
            ax_UV.plot(tfb, Lum_UV, label = f'This work' if k == 2 else None, c = colors_obs[k], linewidth = 3)
            ax_Xray.plot(time_MG, Lum_XrayMG, c = colors_obs[k], ls = '--' if group == 'bandsMG' else '-', linewidth = 3)
            if group == 'bands':
                ax_op.scatter(tfb[np.argmax(Lum_op)], np.max(Lum_op), c = colors_obs[k], s = 200, marker = 'd')
                ax_UV.scatter(tfb[np.argmax(Lum_UV)], np.max(Lum_UV), c = colors_obs[k], s = 200, marker = 'd')
                ax_Xray.scatter(time_MG[np.argmax(Lum_XrayMG)], np.max(Lum_XrayMG), c = colors_obs[k], s = 200, marker = 'd')
    ...
    for ax in axes:
        if group != 'bandsMG':
            ax.plot(tfb, Lum_fld, c = 'k', ls = '-.', linewidth = 2)
            ax.axvline(x = tfb[idx_maxL], c = 'gray', ls = '-.', linewidth = 2)
        ax.set_yscale('log')
        ax.set_xticks(new_ticks)
        ax.set_xlabel(r't / t$_{\rm fb}$', fontsize = 30)
        ax.set_xticklabels(labels)
