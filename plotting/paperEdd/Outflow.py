...
for i, snap in enumerate(snaps):
    ...
    if kind_of_plot == 'convergenceOE':
        ...
        for_title = 'OE'
    elif kind_of_plot == 'convergencebern' or kind_of_plot == 'ratioE':
    ...
    if kind_of_plot == 'ratioE': # slides of boundness/unboundness with velocity arrows, (if how_amy==3, also time evolution of velocity)
        ...
        ax2.set_xlabel(r'X ($r_{\rm t}$)', fontsize = 40)
        ax2.set_ylabel(r'Z ($r_{\rm t}$)', fontsize = 40)
        ax2.set_xlim(xlim_min, xlim_max) 
        ax2.set_ylim(ylim_min, ylim_max)
        ax2.text(.9*xlim_min, 0.85*ylim_max, f't = {np.round(tfb[i],2)}' + r' t$_{\rm fb}$', color = 'k', fontsize = 40)
        ax2.text(.9*xlim_min, 0.85*ylim_min, r'y = 0', fontsize = 40)
        cbar_ax = fig.add_subplot(gs[1, 0])  # Colorbar subplot below the first two
        cb = fig.colorbar(img, orientation='horizontal', cax=cbar_ax, pad=0.18)
        ...
        cb = fig.colorbar(img, orientation='horizontal', cax=cbar_ax, pad = 0.18)
        cb.set_label(r'f$_{\rm unb}$', fontsize = 45)
        cb.ax.tick_params(labelsize=38) 
        ax3.fill_between(tfb[:i+1], np.array(percentile16[:i+1])/Rt, np.array(percentile84[:i+1])/Rt, color = 'gray', alpha = 0.2)
        ax3.set_xlabel(r't $(t_{\rm fb})$', fontsize = 40)
        ax3.set_ylabel(r'Median r$_{\rm ph}$ ($r_{\rm t}$)', fontsize = 40)
        ax3.set_xlim(0.35, 2.25)
        ax3.set_ylim(1, 100)
        ax3.set_yscale('log')
        ax3.grid()       
        ax3.text(.4, 75, f't = {np.round(tfb[i]*t_fall_hour,1)}' + r' hours', color = 'k', fontsize = 40)
    
        for ax in [ax3, ax2]:
            ax.tick_params(axis='both', which='major', width=1.5, length=12, color = 'k', labelsize = 40)
            ax.tick_params(axis='both', which='minor', width=1, length=9, color = 'k')
            # if ax != ax3:
        ax2.scatter(0,0, c= 'k', marker = 'x', s=100)
...
if kind_of_plot == 'time_evolution' or kind_of_plot == 'convergenceOE' or kind_of_plot == 'convergencebern': # only evolution of velocity/boundness in Fid and High res
    ...
    # plt.plot(tfb, percentile16 * conversion_sol_kms * 1e-4, c = '#b2df8a', alpha = 0.1, linestyle = '--')
    # plt.plot(tfb, percentile84 * conversion_sol_kms * 1e-4, c = '#b2df8a', alpha = 0.1, linestyle = '--')
    # plt.fill_between(tfb, percentile16 * conversion_sol_kms * 1e-4, percentile84 * conversion_sol_kms * 1e-4, color = '#b2df8a', alpha = 0.1)
    ...
    if kind_of_plot == 'convergenceOE' or kind_of_plot == 'convergencebern':
        ...
        ax.plot(tfbL, ratio_unbound_phL, c = '#d95f02', label = 'Low')
        # plt.scatter(tfbH, mean_velH * conversion_sol_kms * 1e-4, c = ratio_unbound_phH, s = 20, vmin = 0, vmax = 0.8, marker = 's', label = 'High')
        # plt.plot(tfbH, percentile16H * conversion_sol_kms * 1e-4, c = '#7570b3', alpha = 0.1, linestyle = '--')
        # plt.plot(tfbH, percentile84H * conversion_sol_kms * 1e-4, c = '#7570b3', alpha = 0.1, linestyle = '--')
        # plt.fill_between(tfbH, percentile16H * conversion_sol_kms * 1e-4, percentile84H * conversion_sol_kms * 1e-4, color = '#7570b3', alpha = 0.1)
        ax.plot(tfbM, ratio_unbound_phM, c = '#b2df8a', label = 'Middle') #c = mean_velH/v_esc, s = 20, vmin = 0.2, vmax = 1)
        # plt.savefig(f'{abspath}/Figs/next_meeting/velPh_conv.png')
        ax.plot(tfb, ratio_unbound_ph, c = '#7570b3', label = 'High') #c = mean_vel/v_esc, s = 20, vmin = 0.2, vmax = 1)
        ...
        fig.tight_layout()
        fig.savefig(f'{abspath}/Figs/1.paperEdd/f_conv.pdf')
