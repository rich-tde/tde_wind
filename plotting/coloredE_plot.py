""" Plot colormesh of energies from coloredE.py"""
...
R0 = 0.6 * Rt
Rg = prel.G * Mbh/prel.csol_cgs**2
apo = orb.apocentre(Rstar, mstar, Mbh, beta)
print(Rt/Rg, apo/Rg, apo)
...
#Res0 data
datares0 = np.load(f'{path}{res0}/colormapE_Alice/coloredE_{res0}.npy') #shape (3, len(tfb), len(radii))
...
radiires0 = np.load(f'{path}{res0}/colormapE_Alice/coloredE_{res0}_radii.npy')
col_ieres0, col_orb_enres0, col_Rad_denres0 = datares0[0], datares0[1], datares0[2]
# convert to cgs
col_ieres0 *= prel.en_converter/prel.Msol_cgs
col_orb_enres0 *= prel.en_converter/prel.Msol_cgs
col_Rad_denres0 *= prel.en_den_converter
...
#%% Res2 data
datares2 = np.load(f'{path}{res2}/colormapE_Alice/coloredE_{res2}.npy')
...
radiires2 = np.load(f'{path}{res2}/colormapE_Alice/coloredE_{res2}_radii.npy')
col_ieres2, col_orb_enres2, col_Rad_denres2 = datares2[0], datares2[1], datares2[2]

# convert to cgs
col_ieres2 *= prel.en_converter/prel.Msol_cgs
col_orb_enres2 *= prel.en_converter/prel.Msol_cgs
col_Rad_denres2 *= prel.en_den_converter
...
for i,time in enumerate(tfb_res0):
    ...
    rel_rad_time = 2*np.abs((col_Rad_denres1[idx] - col_Rad_denres0[i]) / (col_Rad_denres1[idx] + col_Rad_denres0[i]))
    idx_zero_ieL = np.concatenate([np.concatenate(np.where(col_ieres1[idx]==0)), np.concatenate(np.where(col_ieres0[i]==0))])
    idx_zero_RadL = np.concatenate([np.concatenate(np.where(col_Rad_denres1[idx]==0)), np.concatenate(np.where(col_Rad_denres0[i]==0))])
...
for i in range(len(tfb_res2)):
    ...
    rel_rad_time = 2*np.abs((col_Rad_denres1[idx] - col_Rad_denres2[i]) / (col_Rad_denres1[idx] + col_Rad_denres2[i]))
    # f = interp1d(tfb_res1, col_Rad_denres1, kind='linear', axis = 0, fill_value = 'extrapolate')
    # col_Radtest = f(time)
    # rel_rad_time = 2*np.abs((col_Radtest- col_Rad_denres2[i]) / (col_Radtest+ col_Rad_denres2[i]))
    idx_zero_ieH = np.concatenate([np.concatenate(np.where(col_ieres1[idx]==0)), np.concatenate(np.where(col_ieres2[i]==0))])
    idx_zero_RadH = np.concatenate([np.concatenate(np.where(col_Rad_denres1[idx]==0)), np.concatenate(np.where(col_Rad_denres2[i]==0))])
...
if free_streaming:
    #Res0 data
    datares0 = np.load(f'{path}{res0}/colormapE_Alice/fscoloredE_{res0}.npy') #shape (3, len(tfb), len(radii))
    tfb_datares0 = np.loadtxt(f'{path}{res0}/colormapE_Alice/fscoloredE_{res0}_days.txt')
    tfb_res0 = tfb_datares0[1]
    radiires0 = np.load(f'{path}{res0}/colormapE_Alice/fscoloredE_{res0}_radii.npy')
    col_Rad_denres0 = datares0[3]
    # Res1 data
    datares1 = np.load(f'{path}{res1}/colormapE_Alice/fscoloredE_{res1}.npy') #shape (3, len(tfb), len(radii))
    tfb_datares1 = np.loadtxt(f'{path}{res1}/colormapE_Alice/fscoloredE_{res1}_days.txt')
    tfb_res1 = tfb_datares1[1]
    radiires1 = np.load(f'{path}{res1}/colormapE_Alice/fscoloredE_{res1}_radii.npy')
    col_Rad_denres1 = datares1[3]
    
    # Res2 data
    datares2 = np.load(f'{path}{res2}/colormapE_Alice/fscoloredE_{res2}.npy')
    tfb_datares2 = np.loadtxt(f'{path}{res2}/colormapE_Alice/fscoloredE_{res2}_days.txt')
    tfb_res2 = tfb_datares2[1]
    radiires2 = np.load(f'{path}{res2}/colormapE_Alice/fscoloredE_{res2}_radii.npy')
    col_Rad_denres2 = datares2[3]

    heigth_text = np.array([1.5e41, 3e41, 3e42])
    colors_indices = ['navy', 'r', 'deepskyblue']

    radiires0_fs = np.repeat([radiires0],len(col_Rad_denres0), axis = 0)
    radiires1_fs = np.repeat([radiires1],len(col_Rad_denres1), axis = 0)
    radiires2_fs = np.repeat([radiires2],len(col_Rad_denres2), axis = 0)

    Lum0_cgs = col_Rad_denres0 * prel.en_den_converter  * prel.c_cgs * 4 * np.pi * (radiires0*prel.Rsol_cgs)**2 
    Lum_cgs = col_Rad_denres1  * prel.en_den_converter * prel.c_cgs * 4 * np.pi * (radiires1*prel.Rsol_cgs)**2 
    Lum2_cgs = col_Rad_denres2  * prel.en_den_converter * prel.c_cgs * 4 * np.pi * (radiires2*prel.Rsol_cgs)**2 
    ...
    Lum_difference2 = []
    for i, time in enumerate(tfb_res2):
        # find the comparable time in res1
        idx = np.argmin(np.abs(tfb_res1 - time))
        denom2 = (Lum_cgs[idx] + Lum2_cgs[i])/2
        Lum_difference2.append(np.abs(Lum_cgs[idx]-Lum2_cgs[i])/denom2)

    indices = [np.argmin(np.abs(tfb_res1-0.5)), np.argmin(np.abs(tfb_res1-0.8)), np.argmin(np.abs(tfb_res1-1.1))]
    img, ax = plt.subplots(1,2, figsize = (20,7))
    for i,idx in enumerate(indices):
        # don't consider the radii where the luminosity is zero
        ...
        radiiLum0_toplot = np.delete(radiires0_fs[idx], where_zero0)
        radiiLum1_toplot = np.delete(radiires1_fs[idx], where_zero)
        radiiLum2_toplot = np.delete(radiires2_fs[idx], where_zero2)

        Lum_difference0_toplot =  np.delete(Lum_difference0[idx], where_zero0) 
        Lum_difference2_toplot = np.delete(Lum_difference2[idx], where_zero2) 
    ...
    # plt.suptitle('Inside the small box', fontsize = 25)
    plt.tight_layout()
    if save:
        plt.savefig(f'{abspath}/Figs/multiple/Luminositymixed_box.pdf')
    plt.show()
