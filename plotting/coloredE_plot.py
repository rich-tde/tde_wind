...
radiires1 = np.load(f'{path}{res1}/colormapE_Alice/radiiEn_{res1}.npy')
...
img = ax[2].pcolormesh(radiires1/apo, tfb_res1, col_Radres1, norm=colors.LogNorm(vmin=1e4, vmax= 7e9), cmap = 'viridis')
cb = fig.colorbar(img)
...
for i in range(len(tfb_res0)):
    ...
    rel_rad_time = 2*np.abs((col_Radres1[idx] - col_Radres0[i]) / (col_Radres1[idx] + col_Radres0[i]))
    idx_zero_ieL = np.concatenate([np.concatenate(np.where(col_ieres1[idx]==0)), np.concatenate(np.where(col_ieres0[i]==0))])
    idx_zero_RadL = np.concatenate([np.concatenate(np.where(col_Radres1[idx]==0)), np.concatenate(np.where(col_Radres0[i]==0))])
...
for i in range(len(tfb_res2)):
    ...
    rel_rad_time = 2*np.abs((col_Radres1[idx] - col_Radres2[i]) / (col_Radres1[idx] + col_Radres2[i]))
    idx_zero_ieH = np.concatenate([np.concatenate(np.where(col_ieres1[idx]==0)), np.concatenate(np.where(col_ieres2[i]==0))])
    idx_zero_RadH = np.concatenate([np.concatenate(np.where(col_Radres1[idx]==0)), np.concatenate(np.where(col_Radres2[i]==0))])
...
cb.set_label('Relative difference Low-Middle', fontsize = 25)
...
cb = fig.colorbar(img)
cb.ax.tick_params(labelsize=20)
cb.set_label('Relative difference Middle-High', fontsize = 25)
...
for i in range(2):
    for j in range(2):
        ...
        if i == 1:
            ax[i][j].set_xlabel(r'$R/R_a$', fontsize = 25)
        if j == 0:
            ax[i][j].set_ylabel(r't/t$_{fb}$', fontsize = 25)

plt.tick_params(axis = 'both', which = 'both', direction='in', labelsize = 20)
plt.tight_layout()
if save:
    plt.savefig(f'/Users/paolamartire/shocks/Figs/multiple/orbE_IE_relative_diff.pdf')
plt.show()

#%%
plt.figure(figsize = (7,5))
plt.plot(tfb_res0, median_rel_orbL, label = r'$\Delta_{OE}$ Low-Middle', c = 'navy')
plt.plot(tfb_res2, median_rel_orbH, '--', label = r'$\Delta_{OE}$ Middle-High', c = 'navy')
...
plt.tight_layout()
if save:
    plt.savefig(f'/Users/paolamartire/shocks/Figs/multiple/orbE_IE_diffTime.pdf')
plt.show()

#%% Relative errors for the radiation energy density
fig, ax = plt.subplots(2,1, figsize = (7,10))
img = ax[0].pcolormesh(radiires0/apo, tfb_res0, rel_Rad_absL, cmap=cmap, norm=norm_Rad)
cb = fig.colorbar(img)
cb.set_label('Relative difference Low-Middle', fontsize = 22)

img = ax[1].pcolormesh(radiires2/apo, tfb_res2, rel_Rad_absH, cmap=cmap, norm=norm_Rad)
cb = fig.colorbar(img)
cb.set_label('Relative difference Middle-High', fontsize = 22)

for i in range(2):
    ax[i].axvline(Rt/apo, linestyle ='dashed', c = 'white', linewidth = 0.8)
    ax[i].set_xscale('log')
    ax[i].set_ylabel(r't/t$_{fb}$', fontsize = 25)
ax[1].set_xlabel(r'$R/R_a$', fontsize = 25)

plt.tick_params(axis = 'both', which = 'both', direction='in')
plt.tight_layout()
if save:
    plt.savefig(f'/Users/paolamartire/shocks/Figs/multiple/Rad_relative_diff.pdf')
plt.show()
