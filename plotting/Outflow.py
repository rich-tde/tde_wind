...
ylim_min, ylim_max = -1.4, 1.4
zlim_min, zlim_max = -2, 2
for i, snap in enumerate(snaps):
    ...
    percentile16[i] = np.percentile(vel_ph, 16) 
    percentile84[i] = np.percentile(vel_ph, 84)
    ...
    if kind_of_plot == 'ratioE': # slides of boundness/unboundness with velocity arrows, (if how_amy==3, also time evolution of velocity)
        ...
        yph_yz, zph_yz, Vyph_yz, Vzph_yz, rph_yz = make_slices([y_ph, z_ph, Vy_ph, Vz_ph, r_ph], yz_ph)
        ...
        img = ax1.scatter(x_mid/apo, y_mid/apo, c = bern_mid, s = 20, cmap = 'coolwarm', alpha = 0.5, vmin = -1, vmax = 1)
        ...
        ax1.text(.9*xlim_min, 0.8*ylim_max, f't = {np.round(tfb[i],2)}' + r' t$_{\rm fb}$', color = 'k', fontsize = 26)
        ax1.text(.9*xlim_min, 0.9*ylim_min, r'z = 0', fontsize = 25)
        ...
        img = ax2.scatter(y_yz/apo, z_yz/apo, c = bern_yz, s = 20, cmap = 'coolwarm', alpha = 0.5, vmin = -1, vmax = 1)
        ...
        ax2.set_ylim(zlim_min, zlim_max)
        ax2.text(.9*xlim_min, 0.8*zlim_max, f't = {np.round(tfb[i]*t_fall_hour,1)}' + r' hours', color = 'k', fontsize = 26)
        ax2.text(.9*xlim_min, 0.9*zlim_min, r'x = R$_{\rm p}$', fontsize = 25)
        ...
        for ax in [ax1, ax2, ax3]:
            ax.tick_params(axis='both', which='major', width=1.5, length=7, color = 'k')
            ax.tick_params(axis='both', which='minor', width=1, length=5, color = 'k')
            if ax != ax3:
...
if kind_of_plot == 'time_evolution' or kind_of_plot == 'convergence': # only evolution of velocity/boundness in Fid and High res
    ...
    plt.plot(tfb, ratio_unbound_ph, c = 'yellowgreen', label = 'Fid') #c = mean_vel/v_esc, s = 20, vmin = 0.2, vmax = 1)
    # cbar = plt.colorbar(img)
    # cbar.set_label(r'mean velocity [v$_{\rm esc} (r_{\rm p})$]')
    plt.xlim(-0.09, 1.8)
    plt.xlabel(r'$t [t_{\rm fb}]$')
    plt.ylabel('Unbound/tot')
    ...
    if kind_of_plot == 'convergence':
        ...
        for j, snap_sH in enumerate(snapsH):
            ...
            ratio_unbound_phH[j] = len(bern_phH[np.logical_and(bern_phH>=0, r_phH!=0)]) / len(bern_phH[r_phH!=0])

        plt.plot(tfbL, ratio_unbound_phL, c = 'C1', label = 'Low')
        ...
        plt.plot(tfbH, ratio_unbound_phH, c = 'darkviolet', label = 'High') #c = mean_velH/v_esc, s = 20, vmin = 0.2, vmax = 1)
        plt.legend(fontsize = 16)
        plt.savefig(f'{abspath}/Figs/next_meeting/velPh_conv.png')
        plt.show()
    else:
        plt.savefig(f'{abspath}/Figs/next_meeting/velPh{check}.png')
        plt.show()
