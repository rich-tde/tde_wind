...
figMdot, axMdot = plt.subplots(1, 1, figsize=(10, 6))
for c, check in enumerate(checks):
        ...
        r_tr_median = np.zeros((len(indices_axis), len(snaps)))
        Mdot_median = np.zeros((len(indices_axis), len(snaps)))
        den_tr_median = np.zeros((len(indices_axis), len(snaps)))
        Vr_tr_median = np.zeros((len(indices_axis), len(snaps)))      
        eta_axis = np.zeros((len(indices_axis), len(snaps)))
        Lum_obs_time = np.zeros((len(indices_axis), len(snaps)))
        ...
        for s, snap in enumerate(snaps): 
                ...
                r_tr_snap[s] = np.median(r_tr[r_tr!=0])
                Mdot_tr = 4 * np.pi * r_tr**2 * den_tr * np.abs(Vr_tr)
                Mdot_snap[s] = np.median(Mdot_tr[Mdot_tr!=0])
                for i, observer in enumerate(indices_axis):
                        Lum_obs_time[i][s] = np.median(Lumph[observer])                       
                        nonzero = r_tr[observer] != 0 
                        # print(f'{label_axis[i]}: percentage non zero Rtr/total = {int(len(nonzero[nonzero==True])/len(nonzero)*100)}%')
                        r_tr_median[i][s] = np.median(r_tr[observer][nonzero])
                        Vr_tr_median[i][s] = np.median(Vr_tr[observer][nonzero])
                        den_tr_median[i][s] = np.median(den_tr[observer][nonzero])
                        Mdot_median[i][s] = np.median(Mdot_tr[observer][nonzero])

                        t_dyn = (r_tr_median[i][s]/Vr_tr_median[i][s])*prel.tsol_cgs/t_fb_days_cgs # you want it in t_fb
                        tfb_adjusted = tfb - t_dyn
                        find_time = np.argmin(np.abs(tfb_fall-tfb_adjusted))
                        eta_axis[i][s] = np.abs(Mdot_median[i][s]/mfall[find_time])
        ...
        if check == 'HiResNewAMR':
                ...
                eta = np.median(eta_axis[:, idx_maxLum])
        ...
        for i, observer in enumerate(indices_axis):
                if label_axis[i] not in [r'$-\hat{\textbf{z}}$']:
                        axTr.plot(tfbs, r_tr_median[i]/Rt, c = colors_res[c], ls = lines_axis[i], label = label_axis[i] if c == 0 else '')
                        axTr.scatter(tfbs[idx_maxLum], r_tr_median[i][idx_maxLum]/Rt, c = colors_res[c], s = 85, marker = 'X')
                        axL.plot(tfbs, Lum_obs_time[i]/Ledd_cgs, c = colors_res[c], ls = lines_axis[i], label = label_axis[i] if c == 0 else '')
                        axL.scatter(tfbs[idx_maxLum], Lum_obs_time[i][idx_maxLum]/Ledd_cgs, c = colors_res[c], s = 85, marker = 'X')
                        axMdot.plot(tfbs, Mdot_median[i]/Medd_sol, c = colors_res[c], ls = lines_axis[i], label = label_axis[i] if c == 0 else '')
                        axMdot.scatter(tfbs[idx_maxLum], Mdot_median[i][idx_maxLum]/Medd_sol, c = colors_res[c], s = 85, marker = 'X')
        
axTr.set_ylabel(r'$r_{\rm tr} [r_{\rm t}]$')
axL.set_ylabel(r'$L_{\rm ph} [L_{\rm Edd}]$')
axMdot.set_ylabel(r'$\dot{M}_{\rm w} [\dot{M}_{\rm Edd}]$')
original_ticks = axTr.get_xticks()
midpoints = (original_ticks[:-1] + original_ticks[1:]) / 2
new_ticks = np.sort(np.concatenate((original_ticks, midpoints)))
for ax in [axTr, axL, axMdot]:
...
axMdot_total.set_ylabel(r'$\dot{M}_{\rm w} [\dot{M}_{\rm Edd}]$')
